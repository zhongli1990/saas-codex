...
### Milestone A: Claude runner (Priority 1)

Tasks:
- [ ] Create `claude-runner/` directory structure
- [ ] Implement FastAPI app with `/health`, `/threads`, `/runs`, `/runs/{id}/events`
- [ ] Implement agent loop with tool use (read_file, write_file, bash, list_files)
- [ ] Implement SSE streaming with buffer replay
...
### Milestone B: UI uplift (Priority 2)

Tasks:
- [ ] Add workspace import panel (GitHub URL / local path)
- [ ] Add workspace dropdown
- [ ] Add session list panel
- [ ] Add runner selector (Codex / Claude)
- [ ] Replace raw JSON textarea with transcript panel (Markdown)
- [ ] Add raw events panel (toggleable)
- [ ] Add react-markdown and syntax highlighting
- [ ] Wire up to new backend endpoints

Acceptance criteria:
- Can import workspace from GitHub URL
- Can select workspace and see sessions
...
### Milestone C: SSE E2E (Priority 3)

Tasks:
- [ ] Verify SSE headers in runner (already done for Codex)
- [ ] Verify SSE headers in Claude runner
- [ ] Verify SSE proxy in backend
- [ ] Verify SSE proxy in Next.js route
- [ ] Add E2E test for SSE streaming
...
### Milestone D: Persistence (Priority 4)

Tasks:
- [ ] Create Alembic migration for workspaces, sessions, runs, run_events tables
- [ ] Add SQLAlchemy models
- [ ] Update backend to persist workspaces on import
- [ ] Update backend to persist sessions on create
- [ ] Update backend to persist runs and events on prompt

Acceptance criteria:
- Data persists across container restarts
- Can query past runs and events

### Milestone E: Workspace and session endpoints (Priority 5-6)

Tasks:
- [ ] Implement `POST /api/workspaces/import`
- [ ] Implement `GET /api/workspaces`
- [ ] Implement `GET /api/workspaces/{id}/sessions`
- [ ] Implement `POST /api/sessions` with workspace_id and runner_type
...
### Milestone F: Microservices (Priority 7)

Tasks:
- [ ] Create prompt-manager service (FastAPI)
- [ ] Create evaluation service (FastAPI)
- [ ] Create memory service (FastAPI)
- [ ] Create llm-gateway service (FastAPI)
- [ ] Add all to docker-compose.yml
- [ ] Implement basic CRUD/query endpoints
...
## 15. Files changed in this session (pre-dev-branch)

The following files have been modified and should be committed to the dev branch:
